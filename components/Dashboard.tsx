...
type Tab = 'resumo' | 'checklist' | 'veiculo' | 'custos' | 'finalizar';
...
${tripData.generalObservations || 'Nenhuma.'}
  ...
  const TabButton: React.FC<{ tab: Tab; label: string; icon: React.ReactNode }> = ({ tab, label, icon }) => (
    <button
      ...
      className={`flex flex-col items-center justify-center p-2 flex-1 transition-colors ${activeTab === tab ? 'text-blue-400 border-t-2 border-blue-400 bg-gray-800' : 'text-gray-400 hover:text-gray-200 hover:bg-gray-800/50'}`}
    >
            ...
            {activeTab === 'finalizar' && (
                <div className="space-y-6 fade-in">
                    ...
                    <div className="bg-gray-800 p-6 rounded-lg shadow-lg border-t-4 border-indigo-500">
                         ...
                         <p className="text-gray-400 mb-6">Certifique-se de que todos os dados, despesas e checklists foram preenchidos corretamente antes de enviar.</p>
                         <div className="flex flex-col gap-4">
                             ...
                             <button onClick={handleSendReport} className="w-full py-3 px-4 bg-indigo-600 text-white font-bold rounded-lg shadow-md hover:bg-indigo-700 transition-colors">
                                 Finalizar Viagem
                             </button>
                         </div>
                    </div>
